                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {recipes.map(recipe => (
                        <div key={recipe.id} className="card bg-base-100 shadow-xl">
                            {recipe.photo && (
                                <figure>
                                    <mig src={recipe.photo} alt={recipe.title} className="h-48 w-full object-cover" />
                                </figure>
                            )}
                            <div className="card-body">
                                ...
                                <div className="card-actions justify-end mt-4">
                                    <Link to={`/recipes/${recipe.id}`} className="btn btn-sm btn-outline">View</Link>
                                    <Link to={`/recipes/${recipe.id}/edit`} className="btn btn-sm btn-outline">Edit</Link>
                                    <button onClick={() => handleDelete(recipe.id)} className="btn btn-sm btn-error btn-outline">Delete</button>
                                </div>
                        </div>
                    </div>
                    ))}
